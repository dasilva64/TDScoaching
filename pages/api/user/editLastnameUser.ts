...
export default withIronSessionApiRoute(
  async function editLastnameUser(req, res) {
    if (req.method === "POST") {
      if (req.session.user) {
        const { lastname, pseudo } = await req.body;
        ...
        if (pseudo.trim() !== "") {
          return res.status(400).json({
            ...
            type: "error",
            message:
              "Vous ne pouvez pas modifier votre nom de famille, veuillez réessayer plus tard",
          });
        } else {
