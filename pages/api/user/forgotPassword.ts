  ...
  if (req.method === "POST") {
    ...
    if (user === null) {
      return res.status(404).json({
        status: 404,
        message: "Aucun compte n'est associé à cette adresse email, veuillez réessayer",
      });
    } else {
      if (user.resetToken) {
        return res.status(404).json({
          status: 404,
          message:
            "Un lien de réinitialisation à déjà été envoyé à cette adresse, veuillez réessayer",
        });
      } else {
