...
const DatePicker = ({ events }: any) => {
  /*  const {
    data: allMeeting,
    isLoading,
    isError,
  } = useGet("/api/meeting/getAllAfterNow"); */
  const [allMeeting, setAllMeeting] = useState<any>(null);
  ...
  useEffect(() => {
    if (dataDelete) {
      if (dataDelete.status === 200) {
        ...
        dispatch({
          type: "flash/storeFlashMessage",
          payload: { type: "error", flashMessage: dataDelete.message },
        });
        resetDelete();
      }
    }
  }, [dataDelete, dispatch, resetDelete]);
  ...
  useEffect(() => {
    ...
    if (isMobile === false && events) {
      if (startDateWeek === "") {
        setAllMeeting(events);
        ...
        events.length > allMeeting.length ||
        events.length < allMeeting.length
      ) {
        setAllMeeting(events);
        let current = new Date();
  ...
  }, [arDateWeek, choiceDate, isMobile, startDateWeek, events, allMeeting]);
  const changeDate = (wowtets: any) => {
    const getMeetingByWeek = (newar: any) => {
      let ar = [];

      let copyEvents: any = [...events];
      for (let y = 0; y < newar.length; y++) {
        let arWeek = [];
        for (let i = 0; i < copyEvents.length; i++) {
          let frDate = new Date(copyEvents[i]["startAt"]).toLocaleString(
          ...
          <div className={styles.datePickermobile__headMobile}>
            ...
            <div className={styles.datePickermobile__headMobile__div}>
              ...
              >
                ...
                <div
                  className={styles.datePickermobile__headMobile__div__div__i}
                >
              ...
              >
                <p className={styles.datePickermobile__headMobile__div__div__p}>
                  {choiceDate.getFullYear()}
                </p>
                <div
                  className={styles.datePickermobile__headMobile__div__div__i}
          ...
          <table className={styles.datePickermobile__table}>
            <thead className={styles.datePickermobile__table__thead}>
              <tr className={styles.datePickermobile__table__thead__tr}>
                <th
                  className={styles.datePickermobile__table__thead__tr__th__day}
                >
                  ...
                  if (p[1] !== null) {
                    if (p[1]["userId"] === id) {
                      return (
                        <tr
                          key={index}
                          className={styles.datePickermobile__table__tbody__tr}
                        >
                          <td
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <div
                              className={
                                styles.datePickermobile__table__tbody__tr__td__meeting__other
                              }
                            >
                              <p
                                className={
                              ...
                              >
                                {p[1].User.firstname} {p[1].User.lastname}
                              </p>
                            </div>
                          </td>
                        </tr>
                      ...
                      >
                        <div
                          ...
                          onClick={() => {
                            ...
                            if (
                              new Date(create).getTime() > new Date().getTime()
                            ) {
                              dispatch({
                                type: "ModalAddMeetingAdmin/open",
                                payload: {
