...
export async function POST(request: NextRequest) {
    try {
        const rateLimitResponse = await checkRateLimitShort(request, 'rlflx-meet-resume');
                ...
                if (user.meetingId === null) {
                    ...
                    try {
                        await prisma.$transaction(async (tx) => {
                            if (user?.offre_test?.type === "unique" && user.offre_test.hasCard === false) {
                                const editMeet = await prisma.user.update({
                                    where: { id: user?.id },
                                    data: { meetingId: null, offreId: null }
                                })
                            } else {
                                const editOffre = await prisma.offre_test.update({
                                    where: { id: user?.offreId! },
                                    data: { currentMeetingId: null, currentNumberOfMeeting: user?.offre_test?.currentNumberOfMeeting! -1 }
                                })
                                const editMeet = await prisma.user.update({
                                    where: { id: user?.id },
                                    data: { meetingId: null }
