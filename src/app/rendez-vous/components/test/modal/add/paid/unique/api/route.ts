  ...
  try {
    ...
    if (session.isLoggedIn === true) {
      ...
      if (user === null) {
        ...
        const { start, typeCoaching, pseudo, formule } = (await request.json()) as {
          start: string;
          typeCoaching: string;
          pseudo: string;
          formule: string
              ...
              return NextResponse.json(
                {
                  status: 404,
                  message:
                    "Vous avez déjà une offre en cours",
                },
                {
                  status: 404,
                }
                  ...
                  line_items: [
                    {
                      price_data: {
                        currency: "eur",
                        product_data: {
                          name: formule !== 'flash' ? "Rendez-vous unique" : "Offre flash",
                          description: formule === "unique" ? `
              Date: ${new Date(start).toLocaleString("fr")} - 
              Type de coaching: ${typeCoaching}
            ` : `
              1er rendez-vous: ${new Date(start).toLocaleString("fr")} - 
              Type de coaching: ${typeCoaching} - 3×1h + bilan offert
            `,
                        },
