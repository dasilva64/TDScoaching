...
export async function POST(request: NextRequest) {
  try {
    const rateLimitResponse = await checkRateLimitShort(request, 'rlflx-meet-add');
              ...
              const { meeting, offre, meetings } = await prisma.$transaction(async (tx) => {
                let meeting = await prisma.meeting_test.create({
                  data: {
                    ...
                    numberOfMeeting: user?.offre_test?.currentNumberOfMeeting === null ? "1" : (user?.offre_test?.currentNumberOfMeeting! + 1).toString(),
                ...
                let offre = await prisma.offre_test.update({
                  ...
                  data: {
                    currentNumberOfMeeting: user?.offre_test?.currentNumberOfMeeting === null ? 1 : user?.offre_test?.currentNumberOfMeeting! + 1,
                ...
                return { meeting, offre, EditUser, meetings };
              });
