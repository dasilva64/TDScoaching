            ...
            try {
              const { offre, meeting } = await prisma.$transaction(async (tx) => {
                ...
                let offre = await prisma.offre_test.update({
                  ...
                  data: {
                    currentNumberOfMeeting: user?.offre_test?.currentNumberOfMeeting ? user.offre_test?.currentNumberOfMeeting === 0 ? 0 : user.offre_test?.currentNumberOfMeeting - 1 : null,
