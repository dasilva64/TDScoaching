...
export async function DELETE(request: NextRequest) {
  try {
    const rateLimitResponse = await checkRateLimitShort(request, 'rlflx-meet-cancel');
      ...
      if (user === null) {
        ...
        if (user.meetingId === null) {
          ...
          if (meeting === null) {
            ...
            if (new Date(meeting.startAt).getTime() < currentDate.setHours(currentDate.getHours() + 5)) {
              return NextResponse.json(
                {
                  status: 400,
            ...
            if (meeting.status === "expired" || meeting.status === "completed") {
              return NextResponse.json(
                {
                  status: 400,
            ...
            try {
              const { offre, meeting } = await prisma.$transaction(async (tx) => {
                ...
                let meeting = await prisma.meeting_test.update({
                  where: { id: user?.meetingId! },
                  data: { status: "cancelled", status_payment: "not_paid" }
                });
              ...
              /* if (offre.type === "unique") {
                ...
                return NextResponse.json(
                  {
                    status: 400,
                    message: "Vous n'avez pas de formule unique, veuillez réessayer",
                  },
                  {
                    status: 400,
                  }
                                         ...
                                         <body>
                                           ...
                                           <div style="width: 100%">
                                             ...
                                             <div style="background: aqua; padding: 50px 0px 50px 20px; border-radius: 20px">
                                               ...
                                               <a style="text-decoration: none; padding: 10px; border-radius: 10px; cursor: pointer; background: orange; color: white" href="https://tdscoaching.fr/rendez-vous" target="_blank">Prendre un rendez-vous</a>
