...
export async function POST(request: NextRequest) {
  try {
    const rateLimitResponse = await checkRateLimitShort(request, 'rlflx-meet-edit');
      ...
      if (user === null) {
        ...
        if (user.meetingId === null) {
          ...
          if (arrayMessageError.length > 0) {
            ...
            if (new Date(user.meeting_test?.startAt!).getTime() < currentDate.setHours(currentDate.getHours() + 11136)) {
              return NextResponse.json(
                {
                  status: 400,
